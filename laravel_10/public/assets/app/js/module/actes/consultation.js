$(document).ready(function() {
    ...
    OffChange('#periode', updateAmounts);
    OffChange('#typeacte_idS', updateAmounts);
    OffChange('#assurance_utiliser', updateAmounts);
    OffChange('#appliq_remise', updateAmounts);

    OffInput('#taux_remise', function () {
        const value = Math.max(0, toNumber(this.value));
    ...
    function getSelectedPrice() {
        ...
        const option = $('#typeacte_idS option:selected');
        ...
        switch ($('#periode').val()) {
            ...
            case '0':
                return toNumber(option.data('prixj'));

            case '1':
                return toNumber(option.data('prixn'));

            case '2':
                return toNumber(option.data('prixf'));
    ...
    function rech_dosier() {
        ...
        api_rech_dossier(
            ...
            function (response) {
                ...
                updateInsuranceVisibility(response.patient);

                loadTypeActes(response.patient.codeassurance);
            },
    ...
    function updateInsuranceVisibility(patient) {
        ...
        const insured = Number(patient.assure) === 1;

        // Affichage des éléments liés à l'assurance
        ...
        // Si le patient n'est pas assuré,
        // la remise doit obligatoirement être appliquée au patient
        if (!insured) {
            $('#assurance_utiliser').val('non');
    ...
    function loadTypeActes(codeAssurance) {
        ...
        $('#div_typeacteS, #div_medecin').hide();
                ...
                actes.forEach(function (acte) {
                    ...
                    $select.append(
                        $('<option>', {
                            value: acte.codgaran,
                            text: acte.libgaran,
                            'data-prixj': acte.prixj,
                            'data-prixn': acte.prixn,
                            'data-prixf': acte.prixf
                        })
                    );
                });
    ...
    function updateAmounts() {
        ...
        calculateAmounts(prix);
    }

    function calculateAmounts(prix) {
        ...
        if (!assuranceUsed || taux < 0 || taux > 100) {
            taux = 0;
        }
         ...
         */
        const assuranceInitiale = Math.round(
            prix * taux / 100
        );

        const patientInitial = prix - assuranceInitiale;

        let assurance = assuranceInitiale;
        let patient = patientInitial;
        ...
        if (remise > 0) {
            ...
            if (applicationRemise === 'assurance') {
    ...
    function addGroup(data) {
        ...
        const $container = $('#div_info_patient');

        $container.empty();

        const assure = Number(data.assure) === 1;
    ...
    function eng() {
        ...
        const patientId = $('#id_patient').val();
        const acteId = $('#typeacte_idS').val();
        const medecinId = $('#medecin_id').val();
        const periode = $('#periode').val();

        const total = toNumber($('#montant_total').val());
        const assurance = toNumber($('#montant_assurance').val());
        const patient = toNumber($('#montant_patient').val());
        const remise = toNumber($('#taux_remise').val());

        /*
         * Validation
         */
        if (!patientId || !acteId || !medecinId || periode === '') {
        ...
        if (assurance + patient + remise !== total) {
        ...
        requestWithCsrf(
            'POST',
            $('#url').attr('content') + '/api/consultations/create',
            {
                id_patient: patientId,
                typeacte_id: acteId,
                user_id: medecinId,
                periode: periode,
                montant_assurance: assurance,
                montant_patient: patient,
                taux_remise: remise,
                total: total,
                appliq_remise: $('#appliq_remise').val(),
                mumcode: $('#mumcode').val() || null,
                assurance_utiliser: $('#assurance_utiliser').val(),
                codeassurance: $('#patient_codeassurance').val() || null,
                patient_numdossier: $('#patient_numdossier').val() || null,
                patient_taux: $('#patient_taux').val() || 0
            }
        )
